...
export async function GET(request: NextRequest) {
  try {
    ...
    const action = searchParams.get("action") ?? "get_alerts";
    ...
    const result = await sendToService("carga", { action, params });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await sendToService("carga", { action: body.action, params: body.params ?? {} });
    return NextResponse.json(result);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Error desconocido";
    return NextResponse.json({ status: "error", message: `Error SOA: ${message}` }, { status: 503 });
